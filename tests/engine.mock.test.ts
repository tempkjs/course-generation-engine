...
describe('CourseEngine (AI_MODE=mock)', () => {
  ...
  it('generateArtefacts throws unless the course is validated (ADR 0004)', async () => {
    const engine = getCourseEngine();
    const course = await engine.generateCurriculum({
      topic: 'Hindustani Vocal toward performance', field: 'arts', level: 'basic',
      audienceExperience: '', durationWeeks: 4, cadence: 'weekend-2x2',
      practitionerId: 'p-1',
      style: { practitionerId: 'p-1', modalities: ['textual'], tone: 'plain', depth: 'working' },
    });
    expect(course.status).toBe('draft');
    await expect(
      engine.generateArtefacts(course.id, ['textual'], {
        practitionerId: 'p-1', modalities: ['textual'], tone: 'plain', depth: 'working',
      }),
