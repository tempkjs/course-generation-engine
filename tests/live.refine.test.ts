...
describe.skipIf(!isLive)('CourseEngine (AI_MODE=live) — refine loop + approval gate', () => {
  it('regenerate produces a visibly different, field-appropriate module; approve opens the gate', async () => {
    ...
    const course = await engine.generateCurriculum({
      topic: 'Hiring & Interviewing Well', field: 'hr', level: 'basic',
      audienceExperience: 'first-time hiring manager', durationWeeks: 4,
      cadence: 'weekend-2x2', practitionerId: 'p-live-refine',
      style: { practitionerId: 'p-live-refine', modalities: ['textual'], tone: 'plain', depth: 'working' },
    });
    expect(course.status).toBe('draft');
    const targetModule = course.modules[0]!;

    const refined = await engine.refineCurriculum(course.id, [
      { op: 'regenerate', nodeId: targetModule.id, instruction: 'reframe around structured interviewing' },
    ]);
    const regenerated = refined.modules.find((m) => m.id === targetModule.id)!;
    ...
    const style: StyleProfile = { practitionerId: 'p-live-refine', modalities: ['textual'], tone: 'plain', depth: 'working' };
    await expect(engine.generateArtefacts(refined.id, ['textual'], style)).rejects.toThrow(/validated/);

    const approved = await engine.approveCurriculum(refined.id);
    expect(approved.status).toBe('validated');

    const artefacts = await engine.generateArtefacts(approved.id, ['textual'], style);
