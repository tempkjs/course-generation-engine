...
export default function Harness() {
  ...
  async function run() {
    const c = await client.generateCurriculum({
      topic, field: 'software', level: 'medium', audienceExperience: '3-5 yrs',
      durationWeeks: 5, cadence: 'weekend-2x2', practitionerId: 'p-demo',
      style: { practitionerId: 'p-demo', modalities: ['textual', 'slide'], tone: 'plain', depth: 'working' },
    });
    setCourse(c);
  }
  ...
  return (
    <main style={{ padding: 24, maxWidth: 720 }}>
      ...
      <button onClick={run} style={{ marginTop: 8, padding: '8px 16px' }}>Generate</button>
      {course && (
        <pre style={{ background: '#f5f5f5', padding: 12, marginTop: 16, overflow: 'auto' }}>
