import type { KnowledgeRetriever, KnowledgeWriter, RetrieveQuery, KnowledgeUnit } from '@/contracts';
...
export class MockKnowledgeCache implements KnowledgeRetriever, KnowledgeWriter {
  async retrieve(query: RetrieveQuery): Promise<KnowledgeUnit[]> {
    ...
    return [...store.values()]
      .filter((u) => u.field === query.field && (!query.level || u.level === query.level))
      .slice(0, query.topK);
  }
  async upsert(unit: KnowledgeUnit): Promise<void> {
    assertServerOnly('KnowledgeCache.upsert');
    store.set(unit.id, { ...unit, reuseCount: (store.get(unit.id)?.reuseCount ?? 0) }); // flywheel signal
  }
}
