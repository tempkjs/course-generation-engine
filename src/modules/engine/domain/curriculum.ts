...
export function assertDraftForRefine(course: Course): void {
  if (course.status !== 'draft') {
    throw new Error(`refineCurriculum is only permitted on a 'draft' course — got '${course.status}'`);
  }
}
...
export function assertApprovable(course: Course): void {
  if (!canTransition(course.status, 'validated')) {
    throw new Error(`approveCurriculum requires a 'draft' course (ADR 0009) — got '${course.status}'`);
  }
}
          ...
          return {
            ...
            lessons: m.lessons.map((l) => {
              ...
              return {
                ...
                ...(edit.patch.objectives ? { objectives: edit.patch.objectives } : {}),
              };
            }),
          };
        });
...
export function findRefineTarget(course: Course, nodeId: string): RefineTarget | undefined {
  const module = course.modules.find((m) => m.id === nodeId);
  if (module) return { kind: 'module', currentTitle: module.title, currentSummary: module.summary };

  for (const m of course.modules) {
    const lesson = m.lessons.find((l) => l.id === nodeId);
    if (lesson) return { kind: 'lesson', currentTitle: lesson.title, currentObjectives: lesson.objectives };
  }
  return undefined;
}
...
export function applyRegeneratedNode(course: Course, nodeId: string, result: RefineResult): Course {
