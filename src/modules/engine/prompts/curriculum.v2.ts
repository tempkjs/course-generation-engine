// Phase-1 curriculum prompt v2 — versioned per prompt-governance rule (never edited in place;
// v1 stays as-is for history). Supersedes v1 on one decision (ADR 0018): a course request can
// carry an optional jurisdiction anchor, threaded into the prompt so curriculum structure and
// wording (module/lesson naming, which regulatory bodies/processes get referenced) grounds
// natively in that jurisdiction rather than defaulting to whichever jurisdiction is
// best-represented in the model's training data for this field.
...
${jurisdictionInstruction(req.jurisdiction)}
