// Phase-2 artefact-content prompt v3 — versioned per prompt-governance rule (never edited in
// place; v1/v2 stay as-is for history). Supersedes v2 on one decision (ADR 0018): content can
// carry an optional jurisdiction anchor (from the course, see Course.jurisdiction), threaded
// into the prompt so legal/regulatory content grounds NATIVELY in that jurisdiction's own
// statutory framework, rather than defaulting to whichever jurisdiction is best-represented
// in the model's training data for this field — the exact failure the Employee Relations live
// run exposed (US law generated for an Indian HR audience with no jurisdiction anchor at all).
//
// Decision 2 (prevention, not detection) and Decision 1 (in-generation claim flagging), both
// from ADR 0013, are unchanged from v2 and still apply in full — jurisdiction grounding
// changes WHICH law gets cited, not whether it gets flagged.
import type { ArtefactType, StyleProfile } from "@/contracts";
import { jurisdictionInstruction } from "./jurisdiction";

export const ARTEFACT_PROMPT_VERSION = "artefacts.v3";
...
${jurisdictionInstruction(ctx.jurisdiction)}
